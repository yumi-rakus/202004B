...
class FavoriteServiceTest {
	...
	@Test
	void createのテスト() throws Exception {
	...
	@Test
	void showAllのテスト() throws Exception {
		List<Favorite> favoriteList = favoriteService.showAll(1);
	...
	@Test
	void deleteのテスト() throws ParseException {
		boolean hasDeleted1 = favoriteService.delete(new Favorite(1, 1, sdf.parse("20200401")));
		assertEquals(true, hasDeleted1, "期待される結果と異なります。");

		boolean hasDeleted2 = favoriteService.delete(new Favorite(2, 1, sdf.parse("20200401")));
		assertEquals(false, hasDeleted2, "期待される結果と異なります。");
	}

	@Test
	void isRegisteredのテスト() {
		String status1 = favoriteService.isRegistered(1, 1);
		assertEquals("200", status1, "期待される結果と異なります。");

		String status2 = favoriteService.isRegistered(0, 0);
		assertEquals("400", status2, "期待される結果と異なります。");
	}
